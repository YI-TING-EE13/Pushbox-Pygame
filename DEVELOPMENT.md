...
- [ ] **編輯器「一鍵試玩」按鈕**：側邊欄新增 `Play Test`，支援從當前未存檔網格無縫切換至臨時 PlayState，Esc 一鍵返回。
- [ ] **編輯器退出防呆警告**：若關卡被編輯且未儲存，按下 Esc 退出時，彈窗二次確認。
- [ ] **無效操作 Screen Shake 震動**：撞牆、推雙箱時觸發棋盤 3-5 像素的極速抖動。
- [ ] **箱子歸位粒子爆發**：箱子落入目標點時，在該 Cell 觸發 15 幀的綠色發光粒子擴散。
- [ ] **首頁「吸引模式」背景**：主選單背景慢速播放半透明的 AI 自動推箱子動畫。

### 2.3 P3 — 產品化與遊戲感延伸項目
- [ ] **多套關卡主題配色包 (Visual Theme Packs)**：實現經典綠、Nord 冰雪藍、Dracula 暗紫等視覺風格的實時切換與渲染。
- [ ] **關卡鎖定解鎖鏈**：實作關卡解鎖進度（只有通關前一關，下一關卡片才變為 Active 解鎖狀態）。
- [ ] **微縮關卡預覽 (Minimap Preview)**：在關卡選擇器右側，選中某關卡時渲染該地圖的 80x80 超微縮格線圖。
- [ ] **時光倒流插值平滑**：撤銷 (Z) 與重做 (Y) 時，角色與箱子以 100ms 短動畫平滑滑動，而非瞬移。
