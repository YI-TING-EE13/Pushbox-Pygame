...
### 6.3 任務 3：自訂關卡 JSON 匯出/匯入與社群分享 [已完成]
* **目標**：賦予自訂關卡傳播屬性，建立社群玩家的自製關卡分享鏈。
* **做法**：
  - **[已完成]** 實作 `src/pushbox/utils/level_share.py` 純邏輯模組，採用 `"pushbox-level-share-v1"` 版本控制 Schema。
  - **[已完成]** 匯出使用 JSON -> UTF-8 -> `zlib.compress` -> `base64` -> `PBX_` 前綴編碼。
  - **[已完成]** 匯入實施 8 點嚴苛防禦性驗證（長度限制、Rectangular 檢查、5x5 至 20x20 尺寸限制、單玩家限制、箱子目標一致性與外圍封閉牆壁檢查）。
  - **[已完成]** 名稱進行防路徑穿越淨化（Sanitize）與重複重名自動後綴遞增去重（Deduplicate）。
  - **[已完成]** 在 `LevelEditor` 側邊欄整合「匯出關卡 (E)」並支援 Tkinter/PowerShell 剪貼簿自動寫入與手動複製 Dialog。
  - **[已完成]** 在 `LevelSelector` 自訂區域整合「匯入關卡」並支援 Ctrl+V 貼上剪貼簿載入，成功後自動定位分頁與微縮預覽，失敗時顯示玻璃美觀警告且不崩潰。
